...
1. Each enabled physical input has its own IO callback at the same buffer size an output on that transport gets (or the size set for the same device as an output), open only while something listens: another process reads `Myco Mic`, or a monitor is on for an enabled output. Idle, the microphones stay closed and macOS shows no microphone indicator for Myco. A reader that starts finds silence until the inputs are up, then each fades in from silence. Per input: the chosen channels averaged to mono (all of one or two, the first two of more, since an audio interface lists its loopback and mix channels after the physical inputs), gain, mute.
2. All enabled inputs are resampled to 48000 and summed into the mic mix.
3. The mix is written into `Myco Mic` for other apps.
4. Every output reads the same mix through a direct ring at its own buffer size, not through the driver, so the wired path stays at a few milliseconds; with monitor off the ring is drained and dropped. The ring holds one input block plus two of the output's pulls or three milliseconds, whichever is more, and every short read earns one more pull, up to eight, because the input and the output run on two IO threads whose timing drifts apart by more than two pulls now and then. The monitor plays whether or not anything plays into the virtual device.
...
Menu bar item with a borderless window drawn like a popover, both plain AppKit; the app opens no SwiftUI scene window, because a window left on another space makes activation switch to that space. The window hangs from a fixed right edge under the status item and is resized from the SwiftUI content size, so a panel that opens beside the main column grows to the left. Every part of that content has a fixed height, the panel and its divider take the column's measured height, because a view that stretched to fill the window would report the window's height back and the window could never shrink. Column contents: master slider, profile chips (a profile holds every output and input setting by UID; the active one is what the engine plays, and the chip row's cog opens the Profiles panel, which adds, removes and renames profiles, records a global shortcut per profile through Carbon `RegisterEventHotKey`, and lists every device a profile knows, connected or not, with the volume and monitor or mute the profile keeps for it), output list (row: enable toggle, name, transport icon, fallback toggle, volume, buffer size, monitor toggle and gain, sync trim when sync is on, EQ button), input list (row: enable toggle, name, gain, mute, and a channel picker on a device with more than one channel), virtual rate picker, sync toggle, driver status with install/uninstall, settings (pin defaults, launch at login). EQ opens as a panel on the left of the column, one output at a time, with a preset picker over the AutoEq headphone corrections, which ship as `Sources/Myco/Resources/autoeq.json`, written by `scripts/autoeq.py` (`make autoeq`, which leaves out the measurers who publish on squig.link, since those are read live) and copied into `Contents/Resources` by `scripts/bundle.sh`, and over every measurement on squig.link: `Squig` reads the site directory and each site's phone book (cached under Caches for a week, one request per database, eight at a time), one row per headphone and site with the site's variants (volume, firmware, ANC mode) unfolded on demand, Super* Review first because AutoEq builds from that rig. A pick downloads the channel files and hands them to `HeadphoneFit` in MycoDSP, which centres measurement and target at 1 kHz, smooths the difference, caps the boost at 6 dB and fits the ten bands by coordinate descent against the Harman target for the form factor (`target-in-ear.csv`, `target-over-ear.csv`). A headphone measured at several volumes, as the AirPods are, gets one set of band shapes and a gain set per volume step (`EQVolumeStep`, kept in `OutputSettings.eqVolumes`); the engine and the EQ panel move the gains linearly between the two steps nearest the master, so the correction follows the loudness tuning the device applies at that volume. Visual design follows the frontend-design skill.

Settings persist in `UserDefaults`: a list of profiles, the active profile's ID, and the global switches. Every key decodes as optional so a build never drops settings an older one saved; a file from before profiles becomes the Default profile. The engine plans with the enabled outputs that are present; when none is, it plans with the fallback output alone, so the stream never goes nowhere while the headphones are off. The app also watches the launch at login setting and registers or unregisters `SMAppService.mainApp`, which only the shipped bundle ID does.

### Build

Swift package, nine targets: `MycoDriver` (C, dynamic library), `MycoAtomics` (C headers carrying the shared feed layout and the memory orderings both sides use), `MycoDSP` (the sample maths, no CoreAudio), `MycoEngine` (the CoreAudio wrappers, the graph, the model and the driver installer), `Myco` (the SwiftUI executable), and the tests `MycoDSPTests`, `MycoEngineTests` with the signal measurements both assert on in `MycoTestSupport`, and `MycoTests` for the app's own rules such as how squig.link variants group. `swift test --filter "MycoDSPTests|MycoTests"` needs no device and no microphone permission; the engine tests need both. `scripts/bundle.sh` builds and assembles `Myco.app` with `Myco.driver` inside `Contents/Resources`. `Makefile` verbs: `build`, `install`, `uninstall`, `run`.
