...
Menu bar item with a popover, both plain AppKit windows; the app opens no SwiftUI scene window, because a window left on another space makes activation switch to that space. Popover contents: master slider, profile chips (a profile holds every output and input setting by UID; the active one is what the engine plays, and the chip row's cog opens the Profiles window, which adds, removes and renames profiles, records a global shortcut per profile through Carbon `RegisterEventHotKey`, and lists every device a profile knows, connected or not), output list (row: enable toggle, name, transport icon, fallback toggle, volume, buffer size, monitor toggle and gain, sync trim when sync is on, EQ button), input list (row: enable toggle, name, gain, mute), virtual rate picker, sync toggle, driver status with install/uninstall, settings (pin defaults, launch at login). EQ opens in its own window per output, placed to the left of the popover so neither covers the other, with a preset picker over the AutoEq headphone corrections, which ship as `Sources/Myco/Resources/autoeq.json`, written by `scripts/autoeq.py` (`make autoeq`) and copied into `Contents/Resources` by `scripts/bundle.sh`. Visual design follows the frontend-design skill.
