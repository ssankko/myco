#ifndef MIXANIMO_FEED_H
#define MIXANIMO_FEED_H

#include <stddef.h>
#include <stdint.h>

#include "MixanimoAtomics.h"

//  The layout of the `Mixanimo` shared ring, which the driver writes and the app maps read only.
...
_Static_assert(sizeof(MixanimoFeedHeader) <= kFeedHeaderBytes, "the feed header must fit its page");
_Static_assert(offsetof(MixanimoFeedHeader, mWriteFrame) == 64, "the IO slots own their cache line");

//  The whole handshake between the two processes: whoever acquires this position sees every sample
//  written below it.
static inline uint64_t mixanimo_feed_write_frame(const MixanimoFeedHeader *header) {
    return mixanimo_atomic_load_acquire(&header->mWriteFrame);
}

//  Driver, from its IO thread: the block first, then the position that publishes the samples.
static inline void mixanimo_feed_publish(
    MixanimoFeedHeader *header, uint32_t blockFrames, uint64_t writeFrame) {
    atomic_store_explicit(
        (_Atomic uint32_t *)&header->mWriteBlockFrames, blockFrames, memory_order_relaxed);
    mixanimo_atomic_store_release(&header->mWriteFrame, writeFrame);
}

//  Driver, with IO stopped: the ring starts over from zero.
static inline void mixanimo_feed_rewind(MixanimoFeedHeader *header) {
    mixanimo_atomic_store_release(&header->mWriteFrame, 0);
}

//  Driver, once the rest of the header is written: the magic tells a reader the header is whole.
static inline void mixanimo_feed_ready(MixanimoFeedHeader *header) {
