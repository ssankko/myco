...
public struct Equalizer: @unchecked Sendable {
    ...
    public func setBands(_ bands: [BandSettings]) {
        ...
        let next = mixanimo_atomic_load_relaxed(generation) &+ 1
        ...
        mixanimo_atomic_store_release(generation, next)
    }
    ...
    private func adoptPublishedCoefficients() {
        let seen = mixanimo_atomic_load_acquire(generation)
        guard seen != mixanimo_atomic_load_relaxed(appliedGeneration) else { return }
        let slot = Int(seen % 2) * Self.bandCount
        scratch.update(from: published + slot, count: Self.bandCount)
        guard mixanimo_atomic_load_acquire(generation) == seen else { return }
        for band in 0..<Self.bandCount {
            for channel in 0..<channels {
                filters[band * channels + channel].coefficients = scratch[band]
            }
        }
        mixanimo_atomic_store_relaxed(appliedGeneration, seen)
    }
}
