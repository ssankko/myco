...
extension Array where Element == AudioDevice {
    /// Drops the devices with no UID and Myco's own virtual pair, which the user never routes.
...
/// Myco's mark, the glyph from `Myco.svg`. Dimmed while the driver is not ready.
struct MycoMark: View {
    var isLive = true
