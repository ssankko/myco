#ifndef MYCO_ATOMICS_H
#define MYCO_ATOMICS_H
...
static inline uint64_t myco_atomic_load_acquire(const uint64_t *slot) {
    return atomic_load_explicit((const _Atomic uint64_t *)slot, memory_order_acquire);
}

static inline void myco_atomic_store_release(uint64_t *slot, uint64_t value) {
    atomic_store_explicit((_Atomic uint64_t *)slot, value, memory_order_release);
}

static inline uint64_t myco_atomic_load_relaxed(const uint64_t *slot) {
    return atomic_load_explicit((const _Atomic uint64_t *)slot, memory_order_relaxed);
}

static inline void myco_atomic_store_relaxed(uint64_t *slot, uint64_t value) {
...
static inline int myco_shm_open_read_only(const char *name) {
