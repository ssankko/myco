...
public struct RingBuffer: @unchecked Sendable {
    ...
    public var fillLevel: Int {
        let write = myco_atomic_load_acquire(positions)
        let read = myco_atomic_load_acquire(positions + Self.readSlot)
    ...
    public func write(_ source: UnsafePointer<Float>, frames: Int) -> Int {
        let write = myco_atomic_load_relaxed(positions)
        let read = myco_atomic_load_acquire(positions + Self.readSlot)
        ...
        myco_atomic_store_release(positions, write &+ UInt64(count))
        return count
    }
    ...
    public func read(into destination: UnsafeMutablePointer<Float>, frames: Int) -> Int {
        let read = myco_atomic_load_relaxed(positions + Self.readSlot)
        let write = myco_atomic_load_acquire(positions)
        ...
        if count > 0 {
            ...
            myco_atomic_store_release(positions + Self.readSlot, read &+ UInt64(count))
        }
        if count < frames {
    ...
    public func reset() {
        myco_atomic_store_release(positions, 0)
        myco_atomic_store_release(positions + Self.readSlot, 0)
    }
}
