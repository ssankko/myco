...
/// The driver's `Myco` ring, mapped read only out of POSIX shared memory.
///
...
struct SharedFeed: @unchecked Sendable {
    ...
    private let header: UnsafePointer<MycoFeedHeader>
    ...
    static func open(name: String = SharedFeed.name) throws -> SharedFeed {
        let file = myco_shm_open_read_only(name)
        guard file >= 0 else {
    ...
    init(base: UnsafeMutableRawPointer, bytes: Int) throws {
        let header = UnsafePointer(base.assumingMemoryBound(to: MycoFeedHeader.self))
    ...
    var writeFrame: UInt64 { myco_feed_write_frame(header) }
